...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        ...
        | LangCSharp::method_declaration
        | LangCSharp::constructor_declaration => visit_scope(unit, node, ctxt, namespace, parent),
        LangCSharp::invocation_expression => bind_call(unit, node, ctxt, namespace, parent),
        LangCSharp::identifier => bind_identifier(node, ctxt, SYM_KIND_ALL),
...
fn bind_call<'tcx>(
    ...
    if let Some(function_node) = node.child_by_field(&unit, LangCSharp::field_function) {
    ...
    for child in node.children(&unit) {
        if child.field_id() != LangCSharp::field_function {
            visit_node(unit, &child, ctxt, namespace, parent);
        }
    }
}
