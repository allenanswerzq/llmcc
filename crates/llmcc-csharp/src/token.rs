...
impl LanguageDefinition for LangCSharp {
    #[rustfmt::skip]
    fn initial_scopes<'tcx>(cc: &'tcx CompileCtxt<'tcx>) -> ScopeStack<'tcx> {
        ...
        for primitive in crate::CSHARP_PRIMITIVES {
                ...
                parser
                    ...
                    .map_err(|error| {
                        Error::parse_failed(format!(
                            "failed to initialize tree-sitter-c-sharp parser: {error}"
                        ))
                        .with_operation("parse_source")
                        .with_context("language", "csharp")
                    })?;
                *parser_slot = Some(parser);
            }

            let Some(parser) = parser_slot.as_mut() else {
                return Err(Error::parse_failed("csharp parser was not initialized")
                    .with_operation("parse_source")
                    .with_context("language", "csharp"));
            };
