...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        LangCSharp::compilation_unit => visit_children(unit, node, ctxt, namespace, parent),
        LangCSharp::class_declaration
        | LangCSharp::struct_declaration
        | LangCSharp::record_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Struct, namespace, parent)
        }
        LangCSharp::interface_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Interface, namespace, parent)
        }
        LangCSharp::enum_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Enum, namespace, parent)
        }
        LangCSharp::method_declaration | LangCSharp::constructor_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Method, namespace, parent)
        }
...
fn scope_and_name<'tcx>(
    ...
    let ident = node
        .query(&unit)
        .try_ident_with_field(LangCSharp::field_name)?;
    Some((scope, ident))
}
