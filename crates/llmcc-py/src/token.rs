...
impl LanguageTraitImpl for LangPython {
    /// Block kind with parent context - default to field kind, then node kind
    ...
    fn collect_init_impl<'tcx>(cc: &'tcx CompileCtxt<'tcx>) -> ScopeStack<'tcx> {
        ...
        for prim in crate::PYTHON_PRIMITIVES {
    ...
    /// Detects: @test, @pytest.mark.*, @unittest.*, etc.
    fn is_test_attribute_impl(node: &dyn ParseNode, source: &[u8]) -> bool {
        ...
        attr_text.contains("@test")
            || attr_text.contains("@pytest")
            || attr_text.contains("@unittest")
    }
