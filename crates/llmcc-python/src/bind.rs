...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        LangPython::class_definition | LangPython::function_definition => {
            visit_scope(unit, node, ctxt, namespace, parent)
        }
        LangPython::call => bind_call(unit, node, ctxt, namespace, parent),
        LangPython::identifier => bind_identifier(node, ctxt, SYM_KIND_ALL),
        LangPython::attribute => visit_children(unit, node, ctxt, namespace, parent),
...
fn bind_call<'tcx>(
    ...
    if let Some(function_node) = node.child_by_field(&unit, LangPython::field_function) {
    ...
    for child in node.children(&unit) {
        if child.field_id() != LangPython::field_function {
            visit_node(unit, &child, ctxt, namespace, parent);
        }
    }
}
