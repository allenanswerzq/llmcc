...
impl<'tcx> GraphFixture<'tcx> {
	...
	fn lookup_symbol(
		...
		stack
			...
			.or_else(|| {
				stack.find_global_suffix_with_filters(&[key], None, Some(unit_index))
			})
...
fn single_unit_function_call_creates_edge() {
	...
	assert_eq!(unit_graph.edges().get_depends(caller_block), vec![helper_block]);
	assert_eq!(unit_graph.edges().get_depended(helper_block), vec![caller_block]);
...
fn cross_unit_call_creates_bidirectional_edges() {
	...
	assert!(helper_unit
		.edges()
		.has_relation(helper_block, BlockRelation::DependedBy, caller_block));
...
fn type_reference_creates_dependency_edge() {
	...
	assert!(use_type_unit
		.edges()
		.has_relation(use_type_block, BlockRelation::DependsOn, foo_block));
	assert!(foo_unit
		.edges()
