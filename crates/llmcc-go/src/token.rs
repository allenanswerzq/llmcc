...
impl LanguageDefinition for LangGo {
    #[rustfmt::skip]
    fn initial_scopes<'tcx>(cc: &'tcx CompileCtxt<'tcx>) -> ScopeStack<'tcx> {
        ...
        for primitive in crate::GO_PRIMITIVES {
                ...
                parser
                    ...
                    .map_err(|error| {
                        Error::parse_failed(format!(
                            "failed to initialize tree-sitter-go parser: {error}"
                        ))
                        .with_operation("parse_source")
                        .with_context("language", "go")
                    })?;
                *parser_slot = Some(parser);
            }

            let Some(parser) = parser_slot.as_mut() else {
                return Err(Error::parse_failed("go parser was not initialized")
                    .with_operation("parse_source")
                    .with_context("language", "go"));
            };
