...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        LangJava::program => visit_children(unit, node, ctxt, namespace, parent),
        LangJava::class_declaration | LangJava::record_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Struct, namespace, parent)
        }
        LangJava::interface_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Interface, namespace, parent)
        }
        LangJava::enum_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Enum, namespace, parent)
        }
        LangJava::method_declaration | LangJava::constructor_declaration => {
...
fn scope_and_name<'tcx>(
    ...
    let ident = node
        .query(&unit)
        .try_ident_with_field(LangJava::field_name)?;
    Some((scope, ident))
}
