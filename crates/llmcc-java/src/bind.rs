...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        LangJava::class_declaration
        | LangJava::record_declaration
        | LangJava::interface_declaration
        | LangJava::enum_declaration
        | LangJava::method_declaration
        | LangJava::constructor_declaration => visit_scope(unit, node, ctxt, namespace, parent),
        LangJava::method_invocation => bind_call(unit, node, ctxt, namespace, parent),
        LangJava::identifier => bind_identifier(node, ctxt, SYM_KIND_ALL),
...
fn bind_call<'tcx>(
    ...
    if let Some(function_node) = node.child_by_field(&unit, LangJava::field_function) {
    ...
    for child in node.children(&unit) {
        if child.field_id() != LangJava::field_function {
            visit_node(unit, &child, ctxt, namespace, parent);
        }
    }
}
