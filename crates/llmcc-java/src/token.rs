...
include!(concat!(env!("OUT_DIR"), "/java_tokens.rs"));

impl LanguageDefinition for LangJava {
    #[rustfmt::skip]
    fn initial_scopes<'tcx>(cc: &'tcx CompileCtxt<'tcx>) -> ScopeStack<'tcx> {
        ...
        for primitive in crate::JAVA_PRIMITIVES {
                ...
                parser
                    ...
                    .map_err(|error| {
                        Error::parse_failed(format!(
                            "failed to initialize tree-sitter-java parser: {error}"
                        ))
                        .with_operation("parse_source")
                        .with_context("language", "java")
                    })?;
                *parser_slot = Some(parser);
            }

            let Some(parser) = parser_slot.as_mut() else {
                return Err(Error::parse_failed("java parser was not initialized")
                    .with_operation("parse_source")
                    .with_context("language", "java"));
            };
