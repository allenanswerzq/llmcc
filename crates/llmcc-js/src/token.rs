...
include!(concat!(env!("OUT_DIR"), "/javascript_tokens.rs"));

impl LanguageDefinition for LangJavaScript {
    #[rustfmt::skip]
    fn initial_scopes<'tcx>(cc: &'tcx CompileCtxt<'tcx>) -> ScopeStack<'tcx> {
        ...
        for primitive in crate::JAVASCRIPT_PRIMITIVES {
                ...
                parser
                    ...
                    .map_err(|error| {
                        Error::parse_failed(format!(
                            "failed to initialize tree-sitter-javascript parser: {error}"
                        ))
                        .with_operation("parse_source")
                        .with_context("language", "javascript")
                    })?;
                *parser_slot = Some(parser);
            }

            let Some(parser) = parser_slot.as_mut() else {
                return Err(Error::parse_failed("javascript parser was not initialized")
                    .with_operation("parse_source")
                    .with_context("language", "javascript"));
            };
