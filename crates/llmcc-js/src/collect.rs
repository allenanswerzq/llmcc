...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        LangJavaScript::program => visit_children(unit, node, ctxt, namespace, parent),
        LangJavaScript::class_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Struct, namespace, parent)
        }
        LangJavaScript::function_declaration | LangJavaScript::generator_function_declaration => {
            visit_named_scope(unit, node, ctxt, SymKind::Function, namespace, parent)
        }
        LangJavaScript::method_definition => {
...
fn scope_and_name<'tcx>(
    ...
    let ident = node
        .query(&unit)
        .try_ident_with_field(LangJavaScript::field_name)?;
    Some((scope, ident))
}
