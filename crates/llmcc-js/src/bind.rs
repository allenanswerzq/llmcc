...
fn visit_node<'tcx>(
    ...
    match node.kind_id() {
        ...
        | LangJavaScript::function_declaration
        | LangJavaScript::generator_function_declaration
        | LangJavaScript::method_definition => visit_scope(unit, node, ctxt, namespace, parent),
        LangJavaScript::call_expression => bind_call(unit, node, ctxt, namespace, parent),
        LangJavaScript::identifier | LangJavaScript::property_identifier => {
            bind_identifier(node, ctxt, SYM_KIND_ALL)
        }
...
fn bind_call<'tcx>(
    ...
    if let Some(function_node) = node.child_by_field(&unit, LangJavaScript::field_function) {
    ...
    for child in node.children(&unit) {
        if child.field_id() != LangJavaScript::field_function {
            visit_node(unit, &child, ctxt, namespace, parent);
        }
    }
}
