...
import { getBrowserBridge, browserTools } from './browser.js';
const colors = {
    reset: '\x1b[0m',
    bold: '\x1b[1m',
    dim: '\x1b[2m',
...
export function executeCommand(input, session, config) {
...
`,
        ...
        case '/config':
            const chromeStatus = config.chrome ? `${colors.green}enabled${colors.reset}` : `${colors.dim}disabled${colors.reset}`;
            return {
                session,
                config,
                shouldExit: false,
  ...
  ${colors.green}Chrome:${colors.reset}     ${chromeStatus}
...
`,
        ...
        case '/export':
            ...
            let exportContent = `# llcraft conversation\n`;
        ...
        case '/browser':
            // Toggle chrome mode
            const newChromeState = !config.chrome;
            if (newChromeState) {
                ...
                if (!bridge.isAvailable()) {
                    return {
                        ...
                        config,
                        shouldExit: false,
                        output: `${colors.red}Chrome package not found.${colors.reset}\n${colors.dim}Install with: cd ../chrome && npm install && npm run build${colors.reset}`,
                    };
                }
            }
            return {
                session,
                config: { ...config, chrome: newChromeState },
                shouldExit: false,
                output: newChromeState
                    ? `${colors.green}Browser automation enabled${colors.reset}\n${colors.dim}${browserTools.length} browser tools now available${colors.reset}`
                    : `${colors.dim}Browser automation disabled${colors.reset}`,
            };
        default:
            return {
                session,
