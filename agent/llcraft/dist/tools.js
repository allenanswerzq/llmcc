...
function executeReadFile(filePath, startLine, endLine) {
...
function executeListDir(dirPath) {
...
function executeSearchFiles(pattern, searchPath, filePattern) {
...
function executeEditFile(filePath, oldString, newString) {
    const absolutePath = path.resolve(filePath);
    if (!fs.existsSync(absolutePath)) {
        return `File not found: ${absolutePath}`;
    }
 ...
 */
function executeApplyPatch(filePath, patch, fuzzFactor) {
    const absolutePath = path.resolve(filePath);
    if (!fs.existsSync(absolutePath)) {
        return `File not found: ${absolutePath}`;
    }
 ...
 */
function executeCreatePatch(filePath, oldContent, newContent) {
    try {
