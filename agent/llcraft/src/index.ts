#!/usr/bin/env node
/**
 * llcraft - Persistent code agent REPL with browser automation
 *
 * A REPL that maintains conversation history in a JSONC file.
 * Supports file/shell tools and browser automation via chrome package.
 */
...
import { getBrowserBridge } from './browser.js';

// Default paths
const CONFIG_DIR = process.env.LLCRAFT_CONFIG_DIR || path.join(process.env.HOME || '', '.llcraft');
...
function loadConfig(): Config {
    ...
    if (fs.existsSync(CONFIG_FILE)) {
        ...
        const configContent = `// llcraft configuration
{
  ...
  // Browser automation (requires chrome package)
  "chrome": false,
...
function saveSession(session: Session): void {
    ...
    const content = `// llcraft session - ${session.created}
...
function printWelcome(session: Session, config: Config): void {
    ...
    console.log(`${colors.dim}Commands: /help /clear /history /chrome /save /exit${colors.reset}`);
    console.log();
}

function printHelp(): void {
    console.log(`
${colors.cyan}${colors.bold}llcraft commands:${colors.reset}
  ...
  ${colors.green}/chrome${colors.reset}    Toggle browser automation
  ${colors.green}/system${colors.reset}    Show/set system prompt
  ${colors.green}/exit${colors.reset}      Exit llcraft
...
async function handleInput(
    ...
    try {
        ...
        const onToolCall = (name: string, args: string, thinking?: string) => {
...
async function main(): Promise<void> {
    ...
    // Enable chrome if --chrome flag is passed
    if (args.includes('--chrome') || args.includes('-c')) {
        config.chrome = true;
        // Check if browser bridge is available
        const bridge = getBrowserBridge();
        if (!bridge.isAvailable()) {
            console.log(`${colors.yellow}Warning: chrome package not found. Browser tools will not be available.${colors.reset}`);
