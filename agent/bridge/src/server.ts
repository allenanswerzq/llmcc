...
export class ApiServer {
    ...
    /**
     * Main request handler - Routes incoming HTTP requests to appropriate handlers.
     *
     * ┌─────────────────────────────────────────────────────────────────────────────────────┐
     * │                           REQUEST/RESPONSE FLOW                                      │
     * └─────────────────────────────────────────────────────────────────────────────────────┘
     *
     * 1. CLIENT (Claude Code, Codex CLI, etc.)
     *    │
     *    │  HTTP POST to localhost:5168/v1/chat/completions (OpenAI format)
     *    │  or /v1/messages (Anthropic format)
     *    │
     *    ▼
     * 2. THIS BRIDGE SERVER (ApiServer.handleRequest)
     *    │  - Parses the incoming request (OpenAI or Anthropic format)
     *    │  - Routes to appropriate handler based on URL:
     *    │    • /v1/chat/completions → handleChatCompletions or handleAgentChatCompletions
     *    │    • /v1/messages → handleMessages
     *    │    • /v1/responses → handleResponses (Codex format)
     *    │
     *    ▼
     * 3. HANDLER (e.g., handleChatCompletions in handlers/*.ts)
     *    │  - Converts request to VS Code Language Model API format
     *    │  - Maps model names (e.g., "claude-opus-4" → "claude-sonnet-4")
     *    │
     *    ▼
     * 4. VS CODE LANGUAGE MODEL API (vscode.lm.selectChatModels + model.sendRequest)
     *    │  - vscode.lm.selectChatModels() finds available Copilot models
     *    │  - model.sendRequest() sends the prompt to GitHub Copilot
     *    │  - This is the bridge to GitHub's infrastructure
     *    │
     *    ▼
     * 5. GITHUB COPILOT BACKEND (remote, managed by GitHub)
     *    │  - Receives request via VS Code's authenticated connection
     *    │  - Routes to actual LLM provider (Anthropic, OpenAI, Google, etc.)
     *    │  - Returns streaming response
     *    │
     *    ▼
     * 6. RESPONSE FLOWS BACK (reverse path)
     *    │
     *    │  GitHub Copilot → VS Code LM API (AsyncIterable<LanguageModelTextPart>)
     *    │       ▼
     *    │  Handler converts to OpenAI/Anthropic SSE format
     *    │       ▼
     *    │  Bridge streams chunks back to client via HTTP
     *    │       ▼
     *    │  Client receives standard API response
     *    │
     *    ▼
     * 7. CLIENT receives response as if talking to OpenAI/Anthropic directly
     *
     * ┌─────────────────────────────────────────────────────────────────────────────────────┐
     * │                              KEY INSIGHT                                             │
     * │  This bridge translates between OpenAI/Anthropic HTTP APIs and VS Code's            │
     * │  internal Language Model API, letting external tools use your Copilot subscription. │
     * └─────────────────────────────────────────────────────────────────────────────────────┘
     */
