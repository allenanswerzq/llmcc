...
async function main() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            switch (name) {
                case 'navigate': {
                    result = await browser.navigate(activeTabId, args?.url as string);
                    break;
                }
                ...
                case 'click': {
                    result = await browser.click(activeTabId, args?.x as number, args?.y as number);
                    break;
                }

                case 'type_text': {
                    result = await browser.type(activeTabId, args?.text as string);
                    break;
                }
                ...
                case 'read_page': {
                    const tree = await browser.getAccessibilityTree(activeTabId, args?.depth as number);
                    result = tree;
                    break;
                }

                case 'find_element': {
                    const elements = await browser.findElements(activeTabId, args?.query as string);
                    result = JSON.stringify(elements, null, 2);
                    break;
                }

                case 'execute_javascript': {
                    const jsResult = await browser.executeScript(activeTabId, args?.script as string);
